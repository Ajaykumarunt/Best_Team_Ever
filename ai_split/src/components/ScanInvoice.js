...
const ScanInvoice = () => {
  ...
  useEffect(() => {
    ...
    const mappedProducts = ocrResult.map((item, index) => ({
      ...
      .then(async (docRef) => {
        ...
        userShares.forEach(async (usr) => {
          ...
          const querySnapshot = await getDocs(
            query(usersRef, where("email", "==", usr.user_email))
          );
      ...
      () => {
        getDownloadURL(uploadTask.snapshot.ref).then((downloadURL) => {
          ...
          axios
            ...
            .then((response) => {
              console.log(response.data.receipts[0].items);
              setOcrResult(response.data.receipts[0].items);
              setUploading(false);
            })
            .catch((error) => {
              console.error(error);
            ...
            <>
              ...
              <div className="flex justify-end w-1/2">
                <button
                  ...
                  className="py-3 px-8 bg-light-green rounded-md mt-16 disabled:bg-gray-400 disabled:cursor-not-allowed"
                  disabled={(expenseTitle === "" && selectedGroup === "" && payer === "")}
                >
                  Next
                </button>
              </div>
